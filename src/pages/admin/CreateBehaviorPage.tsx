...
import { setBehaviorActivation } from '../../utils/behaviorSettings';
import { useBehaviorGroupsStore } from '../../store/useBehaviorGroupsStore';
...
export default function CreateBehaviorPage() {
  const navigate = useNavigate();
  const { groupId: groupIdParam } = useParams<{ groupId?: string }>();
  const { notification, showError, showSuccess, hideNotification } = useNotification();
  const { addBehavior } = useBehaviorGroupsStore();
  ...
  const breadcrumbs = useMemo(() => [
    ...
    { label: 'Thêm hành vi', active: true },
  ], [navigate]);

  useEffect(() => {
    if (!groupIdParam) {
      showError('Lỗi', 'Thiếu thông tin nhóm hành vi');
    ...
    const fetchGroup = async () => {
      try {
        const data = await getBehaviorGroupById(Number(groupIdParam));
        setGroup(data);
      } catch (error) {
        console.error('Failed to fetch behavior group', error);
        showError('Lỗi', 'Không thể tải thông tin nhóm hành vi');
        navigate('/admin/gamification/behaviors');
      } finally {
    ...
    fetchGroup();
  }, [groupIdParam, showError, navigate]);
  ...
  useEffect(() => {
    if (pointTypes.length > 0 && behaviorPointTypes.length === 0) {
  ...
  }, [pointTypes, behaviorPointTypes.length]);
  ...
  useEffect(() => {
    ...
    for (const [key, val] of Object.entries(pointTypeSelections)) {
      ...
      if (val.points && !Number.isNaN(parseInt(val.points, 10))) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload: CreateBehaviorRequest = {
      groupId: group.id,
      name: formData.name.trim(),
      frequencyType: formData.frequencyType,
      maxTimesPerFrequency: isFrequencyCountDisabled
        ? 1
        : Number.isNaN(parsedMaxTimes)
          ? 1
          : parsedMaxTimes,
      pointDiligence: parsedDiligence || undefined,
      pointCompetence: parsedCompetence || undefined,
      pointExperience: parsedExperience || undefined,
      behaviorPointTypes: behaviorPointTypes,
    };

    try {
      setSubmitting(true);
      const created = await addBehavior(payload);
      setBehaviorActivation(created.id, isActive);
      showSuccess('Thành công', 'Đã tạo hành vi mới');
      navigate('/admin/gamification/behaviors', { state: { newBehavior: created }, replace: false });
    } catch (error) {
      console.error('Failed to create behavior', error);
      showError('Lỗi', 'Không thể tạo hành vi');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  if (initialLoading) {
    return (
      <div>
        <AdminPageHeader breadcrumbs={breadcrumbs} title="Thêm hành vi" />
        <div className="bg-white rounded-lg shadow p-6 mt-6">
          <p className="text-gray-500">Đang tải thông tin nhóm hành vi...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div>
      <AdminPageHeader breadcrumbs={breadcrumbs} title="Thêm hành vi mới" />
        ...
        <section>
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-gray-200">
            ...
            <button
              ...
              disabled={submitting}
            >
              {submitting ? 'Đang tạo...' : 'Tạo hành vi'}
            </button>
          </div>
        </form>
      </div>
